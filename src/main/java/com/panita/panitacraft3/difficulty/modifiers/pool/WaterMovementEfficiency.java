...
/**
 * This class modifies the water movement efficiency of certain mobs based on the difficulty level.
 */
public class WaterMovementEfficiency implements MobModifier {
    ...
    public boolean canApply(LivingEntity entity) {
        return APPLICABLE_ENTITIES.contains(entity.getType()) &&
                DifficultyCurveUtil.ensureAttributeExists(entity, Attribute.WATER_MOVEMENT_EFFICIENCY, 0.1);
    }

    @Override
    public double apply(LivingEntity entity, double difficulty, double boostRatio, DebugReport debugReport) {
        AttributeInstance attr = entity.getAttribute(Attribute.WATER_MOVEMENT_EFFICIENCY);
