...
/**
 * This class modifies the step height of certain mobs based on the difficulty level.
 */
public class StepHeightModifier implements MobModifier {
    ...
    public boolean canApply(LivingEntity entity) {
        return APPLICABLE_ENTITIES.contains(entity.getType()) &&
                DifficultyCurveUtil.ensureAttributeExists(entity, Attribute.STEP_HEIGHT, 1);
    }

    @Override
    public void apply(LivingEntity entity, double difficulty, double boostRatio, DebugReport debugReport) {
        AttributeInstance attr = entity.getAttribute(Attribute.STEP_HEIGHT);
        if (attr == null) return;
