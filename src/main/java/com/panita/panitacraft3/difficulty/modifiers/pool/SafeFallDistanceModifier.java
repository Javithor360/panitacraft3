...
/**
 * This class modifies the safe fall distance of certain mobs based on the difficulty level.
 */
public class SafeFallDistanceModifier implements MobModifier {
    ...
    public boolean canApply(LivingEntity entity) {
        return APPLICABLE_ENTITIES.contains(entity.getType()) &&
                DifficultyCurveUtil.ensureAttributeExists(entity, Attribute.SAFE_FALL_DISTANCE, 3);
    }

    @Override
    public void apply(LivingEntity entity, double difficulty, double boostRatio, DebugReport debugReport) {
        AttributeInstance attr = entity.getAttribute(Attribute.SAFE_FALL_DISTANCE);
        if (attr == null) return;

        double base = DifficultyCurveUtil.ensureValidBaseValue(attr, 3);
